-- Rollback for 005_product_measurements_batches.sql.
-- This rollback intentionally does not reduce current_stock/minimum_stock precision,
-- because converting numeric(18,6) back to a smaller scale can lose inventory data.
-- It also keeps measurement helper functions because migration 004 already uses
-- them in product_variants constraints and measured product RPCs.
...
alter table public.product_variants
  drop constraint if exists product_variants_tax_percent_check;

alter table public.product_variants
  drop column if exists tax_percent;

-- Restore RPCs to the pre-batch version. The signatures remain unchanged.
